...
| 5  | Elements | `dom`: `StaffElement`, `LineElement`, `StructuralElement`, `NoteElement`, `Clef`, `Articulation`, `ArticulationType`, `AccidentalBounds`, `IntrinsicHeight` | ⏳ | Everything except key state, which pass 0 settled. |
...
| 12 | Song | `dom`: `Song`, `SongMetadata`, `Attribution`, `AttributionFormatter`, `AttributionLine`, `SongAttribution`, `ModificationSession`, `TerminalMaintainer`, `PartialDate` | ⏳ | The large model, less key state, which pass 0 settled. Decouple from everything above it as far as it will go. |
