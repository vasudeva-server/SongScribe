...
**Phase 1 prototypes on `KeySignatureChangeDialog`, not `FontDialog`.** The plan's canonical "small example" is `FontDialog`, but the inventory shows it's pure wiring with zero domain reach — it can't exercise a validate/apply seam. `KeySignatureChangeDialog` is 98 lines and exhibits the whole problem: `getData()` reaches `requireScoreView()` and `getSong().getLine(...)`, `setData()` posts via `postWithModification`. Phase 1 ends at a **checkpoint** — eleven classes get rewritten against that shape, so it's agreed before rollout starts.
