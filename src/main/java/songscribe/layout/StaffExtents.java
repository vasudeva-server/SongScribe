...
public class StaffExtents {
    ...
    /**
     * The value {@link #yGet} reports for a footprint with no reservation. The numeric arrays
     * zero-initialize to the middle staff line, so an untouched step reads back as this default.
     * Callers use it to distinguish "no real reservation" (staff clamp alone applies) from a real
     * support edge.
     */
    public static final double EMPTY_EXTENT_SS = 0.0;
     ...
     * middle-line-relative coordinate system used throughout layout.
     *
     ...
     * <p>
     * Walks the raw arrays, whose {@code 0.0} default means "empty" — an empty region reports
    ...
    /**
     * Copies the top extent array from another {@code StaffExtents} instance. Used when
     * initializing a higher tier from a lower tier's reservations (e.g., structural layer
     * starts from note-attached layer's top extents).
     *
