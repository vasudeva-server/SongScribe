...
public final class LyricEditor extends MyJTextField {
    ...
    public void attachListeners() {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                logState("key ALT-A");
                // Goes through the document filter, so the length limit still applies.
                replaceSelection(e.isShiftDown() ? LONG_A_CAPITAL : LONG_A);
            }
    ...
    private static boolean isLongAKeyPress(KeyEvent event) {
        return event.getKeyCode() == KeyEvent.VK_A
            && event.isAltDown()
            && !event.isControlDown()
            && !event.isMetaDown()
