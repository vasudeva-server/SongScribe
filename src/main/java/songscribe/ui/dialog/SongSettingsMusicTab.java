...
final class SongSettingsMusicTab extends BaseDialog.Tab {
     ...
     * nothing else touched, answers the same tempo and key — and a line width that commits as
    ...
    void populate(SongSettingsInput input) {
        ...
        keyCombo.setSelectedItem(music.key());
    ...
    MusicChoices gather() {
        ...
        return new MusicChoices(tempo, selectedKey(), lineWidthEntry());
    }

    /**
     * The key signature the combo is showing.
     *
     * <p>The combo is built over a fixed non-empty list and is never editable, so it always
     * carries a selection. The guard states that invariant rather than defending against it.
     */
    private KeySelection selectedKey() {
