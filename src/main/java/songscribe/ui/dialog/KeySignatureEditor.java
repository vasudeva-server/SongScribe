 ...
 * It also has two callers — the gesture and {@code KeySignatureChangeAction}'s placement — so
 * constructing the dialog here keeps that two-step in one place.
 */
public final class KeySignatureEditor {

    private KeySignatureEditor() {
    }
    ...
    public static void edit(MainFrame mainFrame, Line line, int insertionIndex) {
        new KeySignatureChangeDialog(mainFrame).showFor(line, insertionIndex);
    }
}
