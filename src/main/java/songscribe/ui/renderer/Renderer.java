...
public abstract class Renderer {
    ...
    // The padding between right info and the right edge of the page
    protected static final int RIGHT_INFO_RIGHT_PADDING = 20;
    ...
    public void drawScore(
        ...
        drawScore(g2, drawEditingComponents, scale, Score.RenderStep.TITLE);
    }
    ...
    public void drawScore(
        ...
        boolean drawEditingComponents,
        double scale,
        Score.RenderStep startStep
    ) {
        /*
          We draw each vertical section of the music sheet separately, with a fixed amount of padding between them:
        ...
        if (startStep == Score.RenderStep.TITLE) {
            // TODO: Eventually this will be calculated
            height = 0;
        ...
        if (startStep.ordinal() <= Score.RenderStep.INFO.ordinal()) {
            drawInfo(g2);
        }

        if (startStep.ordinal() <= Score.RenderStep.COMPOSITION.ordinal()) {
            drawComposition(g2, drawEditingComponents);
        }

        if (startStep.ordinal() <= Score.RenderStep.LYRICS.ordinal()) {
            drawUnderLyrics(g2);
        }

        if (startStep.ordinal() <= Score.RenderStep.FOOTNOTES.ordinal()) {
            drawFootnotes(g2);
        }

        height = score.getRenderMap().get(Score.RenderStep.FOOTNOTES).maxY;
    }
    ...
    private void drawTitle(Graphics2D g2) {
        ...
        if (title.isEmpty()) {
            score.getRenderMap().get(Score.RenderStep.TITLE).maxY = 0;
            lyricsMaxY = 0;
            return;
        }
        ...
        for (var i = 0; i < titleLines.size(); i++) {
            ...
            y = titleMaxAscent + (i * lineHeight);
            drawString(g2, titleLine, x, y);
        }

        // Set the bottom of the last line of the title
        y += metrics.getMaxDescent();
        score.getRenderMap().get(Score.RenderStep.TITLE).maxY = y;
        score.getRenderMap().get(Score.RenderStep.INFO).maxY = y;
        lyricsMaxY = y;
    }

    private void drawInfo(Graphics2D g2) {
        var info = composition.getInfo();

        if (info.isEmpty()) {
            var titleMaxY = score
                .getRenderMap()
                .get(Score.RenderStep.TITLE)
                .maxY;
            score.getRenderMap().get(Score.RenderStep.INFO).maxY = titleMaxY;
            return;
        }

        g2.setFont(composition.getInfoFont());
        var maxWidth = GraphicUtils.getTextBlockWidth(info, g2);
        var x = (float) (composition.getLineWidth() - maxWidth);

        // y is the bottom of the title + padding. Add the ascent of the first line of the right
        // info to get the baseline.
        var y =
            score.getRenderMap().get(Score.RenderStep.TITLE).maxY +
            score.getRenderMap().get(Score.RenderStep.INFO).topPadding +
            g2.getFontMetrics().getAscent();

        y = Math.round(
            drawTextBox(g2, info, x - RIGHT_INFO_RIGHT_PADDING, y, 0)
        );
    ...
    private void drawUnderLyrics(Graphics2D g2) {
        // TODO: Split this up into separate methods for lyrics, Bangla, and translation
        // First measure both the lyrics and translation
        var lyrics = composition.getUnderLyrics();
        var translation = composition.getTranslatedLyrics();

        if (lyrics.isEmpty() && translation.isEmpty()) {
            return;
        }
        ...
        // We will center the lyrics and translation based on the max width
        var lyricsX = (float) Math.round(
            (composition.getLineWidth() - maxWidth) / 2
        );

        if (!composition.getUnderLyrics().isEmpty()) {
            lyricsMaxY = drawTextBox(
                g2,
                lyrics,
                lyricsX,
                (float) score.getUnderLyricsYPos(),
                0f
            );
        }

        if (!composition.getTranslatedLyrics().isEmpty()) {
            lyricsMaxY = drawTextBox(g2, translation, lyricsX, lyricsMaxY, 0);
        }
    }

    private void drawBanglaLyrics(Graphics2D g2) {
        var lyrics = composition.getBanglaLyrics();
        ...
        g2.setFont(font);
        // TODO: Implement drawTextBox for Bangla lyrics
    }
    ...
    private void drawFootnotes(Graphics2D g2) {
        ...
        g2.setFont(font);
        // TODO: Implement drawTextBox for footnotes
    }
