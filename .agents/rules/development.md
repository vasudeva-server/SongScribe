...
- Tests: always `./scripts/compile.sh` first if any file under `src/main/` changed; always `./scripts/test.sh` (never `./gradlew test`). Run unit before e2e. Any e2e test requires user approval. See the usage header in `scripts/test.sh` for targets and flags; it validates them and prints the corrected command when a target is wrong.
