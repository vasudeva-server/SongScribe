...
All user-facing string literals must live in `src/main/resources/songscribe/strings.properties`. No raw string literals for UI text in Java/Kotlin source.
...
label.export.without.lyrics = Export without lyrics under the song
...
label.exportwithoutlyrics = Export without lyrics under the song
```

The generator converts dots to underscores and uppercases everything, so `dialog.composition.settings.reset.to.defaults` becomes the constant `DIALOG_COMPOSITION_SETTINGS_RESET_TO_DEFAULTS` — readable at a glance.

### Retrieving strings

```java
Strings.get(Strings.DIALOG_COMPOSITION_SETTINGS_TITLE)           // simple string
Strings.get(Strings.CONFIRM_FILE_OVERWRITE, fileName)            // MessageFormat template
```
