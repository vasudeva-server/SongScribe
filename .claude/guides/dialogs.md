...
Lifecycle: `getData()` (populate, return false to cancel show), `tabWillShow()`, `tabWillHide()`. A tab has no `dispose()`: it acquires nothing that outlives the dialog's bindings. A button inside a tab is a plain `JButton` with a listener, never a `UIAction`. A `UIAction` registers with the message bus in its constructor and needs an owner to release that registration, and a tab has no disposal point of its own beyond the dialog's bindings — so nothing a tab builds may hold a bus registration, whether the dialog is modal or not. Enablement that depends on the song, such as a button that is meaningless without lyrics, is set once in `populate`; enablement that must follow main-window state while a modeless dialog is open belongs to the dialog's bindings.
