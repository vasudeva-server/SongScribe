...
attaches at the note-head edge or centre, and that it clears the staff lines.
...
- A note's nominal **head box** spans `noteCentreY ± STAFF_POSITION_OFFSET_SS`
  (`STAFF_POSITION_OFFSET_SS = 0.5`) vertically — a half staff-space above and below the note
  centre — and the note-head glyph width horizontally.
...
2. **Vertical seat** `seatSs` = distance from the note centre to the shared endpoint Y, in the
...
3. **Horizontal attachment** — X of each endpoint, derived *from the seat* via the edge/centre step
...
y-tuning). Seat = 0.75 → centre attach. The dot itself never moves.

---

## 6. Horizontal attachment (`tieEndpointXSs`) — the edge/centre step

LilyPond's `get_attachment` samples the chord's **skyline outline at the endpoint's seat height**,
then `note-head-gap` pulls the endpoint toward the span centre (`attachment_x_.widen(-gap)`). That
...
- once the seat has **dropped below** the head box (seat > 0.5), the outline has receded to the
  note-head **centre** (LilyPond's boundary box, `linear_combination(-dir/2)`).
...
endpointX   = skylineX + dir·NOTE_HEAD_GAP_SS  # gap toward the span centre
```

Consequences:

- **Edge attach** left endpoint = `noteLeftX + W + gap` — 0.2 ss into the gap, clear of the note.
- **Centre attach** left endpoint = `noteLeftX + W/2 + gap` — under the note-head, near its centre.
...
reason a tie visibly moves from "inset in the gap" to "centred on the note-heads" as the pitch
changes.
...
1. Find the staff line nearest the natural apex centreline. If none is a real (in-staff) line
...
through it. SongScribe coordinates (sp downward). Note centre Y = `sp × 0.5`; endpoint Y =
`noteCentreY + arcSign × seat`.
