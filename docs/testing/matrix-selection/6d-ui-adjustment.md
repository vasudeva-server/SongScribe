...
| HorizontalAdjustment | `drag()` SINGLE_NOTE: `note.setXOffsetPx(endPoint.x)` | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` TO_END_OF_LINE: all notes from xIndex forward shifted by (endPoint.x - diffX) | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` STRETCH_NOTE_SPACING: each element x = firstX + (stretchHelper[i]-firstX)*ratio; `changeElementSpacingRatio` called | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` START_OF_LINE: all lines' notes shifted by diff from new x | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` GLISSANDO_START: `glissando.x1Translate += endPoint.x - diffX` | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` GLISSANDO_END: `glissando.x2Translate += endPoint.x - diffX` | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` CRESCENDO_START/DIMINUENDO_START: `setX1ShiftSs` incremented by delta | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` CRESCENDO_END/DIMINUENDO_END: `setX2ShiftSs` incremented by delta | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()`: `song.setModified(true)` called on every drag event | unit | none | skip | — | — |
| HorizontalAdjustment | `finishedDrag()`: draggingRect set to null | unit | none | skip | — | — |
| HorizontalAdjustment | `setEnabled(true)`: adjustRects populated with correct types and counts | unit | none | skip | — | — |
| HorizontalAdjustment | `setEnabled(false)`: adjustRects cleared | unit | none | skip | — | — |
| HorizontalAdjustment | `setEnabled(true)`: GLISSANDO_END rect NOT added for SLIDE_OUT glissando | unit | none | skip | — | — |
| HorizontalAdjustment | `findHairpinByAnchor`: returns crescendo/diminuendo matching anchorElementIndex | unit | none | skip | — | — |
| HorizontalAdjustment | `findHairpinByEnd`: returns crescendo/diminuendo matching endElementIndex | unit | none | skip | — | — |
| HorizontalAdjustment | `drag()` diffX calculation: `diffX = draggingRect.rect.x + (rect.width/2)` — midpoint arithmetic | unit | none | skip | — | — |
| VerticalAdjustment | `startedDrag()`: sets startedDrag=false when startPoint=null | unit | none | skip | — | — |
| VerticalAdjustment | `startedDrag()`: sets startedDrag=false when no AdjustRect contains startPoint | unit | none | skip | — | — |
| VerticalAdjustment | `startedDrag()` ROW_HEIGHT: topLeft.y = noteYPosPx(6, 0); bottomRight.y = Integer.MAX_VALUE | unit | none | skip | — | — |
| VerticalAdjustment | `startedDrag()` TEMPO_CHANGE / FIRST_SECOND_ENDING / TRILL / BEAT_CHANGE: upLeft.y = noteYPosPx(6, line-1); downRight.y = noteYPosPx(-4, line) | unit | none | skip | — | — |
| VerticalAdjustment | `startedDrag()` ANNOTATION / TUPLET / CRESCENDO_Y / DIMINUENDO_Y: upLeft.y = noteYPosPx(6, line-1); downRight.y = noteYPosPx(-6, line+1) | unit | none | skip | — | — |
| VerticalAdjustment | `drag()` diffY calculation: `(endPoint.y - dragRect.rect.y) + midPoint.y` | unit | none | skip | — | — |
| VerticalAdjustment | `drag()` diffX calculation: `(endPoint.x - dragRect.rect.x) + midPoint.x` — only used internally, rect.y updated | unit | none | skip | — | — |
| VerticalAdjustment | `adjustAttribution(diffY)`: posts LayoutDidChangeNotification with attributionStartYSs + diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustTopSpace(diffY)`: posts LayoutDidChangeNotification with topPaddingSs + diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustRowHeight(diffY)`: posts LayoutDidChangeNotification with rowHeightAdjustmentSs + diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustTempoChange(line, diffY)`: all TempoChangeAttachment.userYOffsetSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustBeatChange(line, diffY)`: all BeatChangeAttachment.userYOffsetSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustFirstSecondEnding(line, diffY)`: all Ending.yPositionSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustAnnotation(line, diffY)`: both userYOffsetSs AND legacy yPosPx incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustAnnotation(line, diffY)`: no-op when dragRect has no AnnotationAttachment | unit | none | skip | — | — |
| VerticalAdjustment | `adjustTrill(line, diffY)`: all Trill.yPositionSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustDynamics(line, diffY)`: Hairpin.yShiftSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustDynamics(line, diffY)`: no-op when dragRect=null or hairpin not found | unit | none | skip | — | — |
| VerticalAdjustment | `adjustTuplet(line, diffY)`: Tuplet.verticalPositionSs incremented by diffY | unit | none | skip | — | — |
| VerticalAdjustment | `adjustTuplet(line, diffY)`: no-op when tuplet not found | unit | none | skip | — | — |
| VerticalAdjustment | `finishedDrag()`: dragRect set to null | unit | none | skip | — | — |
| VerticalAdjustment | `setEnabled(true)`: ATTRIBUTION rect added only when attribution non-empty | unit | none | skip | — | — |
| VerticalAdjustment | `setEnabled(true)`: TOP_SPACE rect added only when lineCount > 0 | unit | none | skip | — | — |
| VerticalAdjustment | `setEnabled(true)`: ROW_HEIGHT rect added only when lineCount > 1 | unit | none | skip | — | — |
| VerticalAdjustment | `setEnabled(true)`: per-line rects (TEMPO_CHANGE, ANNOTATION, ENDING, TRILL, BEAT_CHANGE, CRESCENDO_Y, DIMINUENDO_Y, TUPLET) populated for matching elements | unit | none | skip | — | — |
| VerticalAdjustment | `setEnabled(false)`: adjustRects cleared | unit | none | skip | — | — |
| VerticalAdjustment | `getAttributionAdjustRect`: x = sheetWidthPx - HANDLE_SIZE_PX; y = attributionStartYSs | unit | none | skip | — | — |
| VerticalAdjustment | `getHeightAdjustRect`: x = 0; y = noteYPosPx(0, line) - HANDLE_SIZE_PX/2 | unit | none | skip | — | — |
| VerticalAdjustment | `getRangeElementAdjustRect`: x = startNote.x - xOffsetPx; y = bounds.topSs - HANDLE_SIZE_PX | unit | none | skip | — | — |
| VerticalAdjustment | `getRangeElementAdjustRect`: returns false and skips when rangeElement/startNote/endNote is null | unit | none | skip | — | — |
| VerticalAdjustment | dynamics handle x = (startX + endX + DYNAMICS_HANDLE_CENTER_BIAS_PX) / 2 | unit | none | skip | — | — |
| VerticalAdjustment | tuplet handle x = startNote.x (upper stem) vs startNote.x + TUPLET_LOWER_HANDLE_X_OFFSET_PX (lower stem) | unit | none | skip | — | — |
| VerticalAdjustment | `findHairpinByAnchor`: returns crescendo/diminuendo matching anchorElementIndex | unit | none | skip | — | — |
| VerticalAdjustment | `getLayoutResultForLine`: throws IllegalStateException when mainPanel null or layoutResult null | unit | none | skip | — | — |
