...
| Song | `canReplaceTerminal` — valid terminal AND differs from current | unit | `SongLineMaintenanceTest.TerminalPredicates` | adequate | write direct predicate test (3 cases) | ✅ |
| Song | `replaceTerminal(type)` — no-op same; replace; throws IAE for non-terminal | unit | `SongLineMaintenanceTest.TerminalPredicates` | missing (error path) | write test: `replaceTerminal(non-terminal)` throws IAE | ✅ |
| Song | `newTerminalElement(type)` — throws IAE for non-terminal | unit | `SongLineMaintenanceTest.TerminalPredicates` | missing | write test asserting IAE | ✅ |
...
| Song | `Song(SongData)` loading ctor — subscribes, no default line | unit | `SongLoadingTest` | inadequate | covered by loadFrom improvement above | ✅ |
